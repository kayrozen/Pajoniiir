...
static const jd9165_lcd_init_cmd_t lcd_cmd[] = {
    ...
    {0x3A, (uint8_t[]){0x55}, 1, 0},    // 16bit 565
    {0x11, (uint8_t[]){0x00}, 1, 120},  // 退出睡眠 延时120ms
    {0x29, (uint8_t[]){0x00}, 1, 20},   // 开启显示 延时20ms
};
 ...
 * @brief 触摸坐标校准回调函数
 * 
 * GT911触摸芯片内部配置的分辨率是800x480,而屏幕实际分辨率是1024x600
 * 此函数将原始触摸坐标线性映射到屏幕坐标
 * 
 * @param tp 触摸句柄
 * @param x X坐标数组(会被原地修改)
 * @param y Y坐标数组(会被原地修改)
 * @param strength 压力值数组
 * @param point_num 触摸点数量
 * @param max_point_num 最大触摸点数
 */
static void touch_coordinate_calibration(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y, 
                                         uint16_t *strength, uint8_t *point_num, uint8_t max_point_num)
{
#if CONFIG_BSP_LCD_TYPE_1024_600
    // 屏幕实际分辨率
    const uint16_t SCREEN_WIDTH = BSP_LCD_H_RES;   // 1024
    const uint16_t SCREEN_HEIGHT = BSP_LCD_V_RES;  // 600
    
    // GT911芯片返回的最大坐标(需要根据实际情况调整)
    const uint16_t TOUCH_MAX_X = 800;
    const uint16_t TOUCH_MAX_Y = 480;
    
    for (uint8_t i = 0; i < *point_num; i++) {
        // 线性校准: Screen = Touch * Screen_Max / Touch_Max
        // 使用整数运算避免浮点,并做边界检查
        uint32_t calibrated_x = ((uint32_t)x[i] * SCREEN_WIDTH) / TOUCH_MAX_X;
        uint32_t calibrated_y = ((uint32_t)y[i] * SCREEN_HEIGHT) / TOUCH_MAX_Y;
        
        // 边界检查,防止溢出
        x[i] = (calibrated_x > SCREEN_WIDTH) ? SCREEN_WIDTH : (uint16_t)calibrated_x;
        y[i] = (calibrated_y > SCREEN_HEIGHT) ? SCREEN_HEIGHT : (uint16_t)calibrated_y;
    }
#endif
}
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    const esp_lcd_touch_config_t tp_cfg = {
        ...
        // 注册坐标校准回调函数
        //.process_coordinates = touch_coordinate_calibration,
    };
    ...
    if (ret == ESP_OK) {
#if CONFIG_BSP_LCD_TYPE_1024_600
        ESP_LOGI(TAG, "Touch calibration enabled: 800x480 -> 1024x600");
#endif
    }
    return ret;
}
