...
void WifiConfigurationAp::Start()
{
    ...
    esp_timer_create_args_t timer_args = {
        .callback = [](void* arg) {
            ...
            if (!self->is_connecting_) {
                ...
                if (ret != ESP_OK) {
                    ESP_LOGW(TAG, "Scan start failed: %s, retrying in 5s", esp_err_to_name(ret));
                    // Scan start may fail if WiFi state not ready yet (ESP_ERR_WIFI_STATE),
                    // retry after 5s instead of waiting for WIFI_EVENT_SCAN_DONE which won't arrive
    ...
    // Start first scan with a 2s delay to allow WiFi to fully stabilize
    // (especially on ESP-Hosted platforms where RPC calls may fail with ESP_ERR_WIFI_STATE)
    esp_timer_start_once(scan_timer_, 2 * 1000000);
}
...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    // Create the default WiFi AP interface and STA interface
...
bool WifiConfigurationAp::ConnectToWifi(const std::string &ssid, const std::string &password)
{
    ...
    // Use longer timeout on P4 (ESP-Hosted mode) where event delivery may be delayed
    TickType_t timeout;
#ifdef CONFIG_SOC_WIFI_SUPPORT_5G
    timeout = pdMS_TO_TICKS(30000);   // 30s for 5G
#elif CONFIG_IDF_TARGET_ESP32P4
    timeout = pdMS_TO_TICKS(20000);   // 20s for P4 ESP-Hosted mode
#else
    timeout = pdMS_TO_TICKS(15000);   // 15s for others
#endif
    ...
    // Fallback for ESP-Hosted mode: events might not reach our handler,
    // query the WiFi driver (slave) directly for connection status
    wifi_ap_record_t ap_info;
    if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
        ESP_LOGW(TAG, "WiFi %s is connected (fallback check), but event was missed", ssid.c_str());
        esp_wifi_disconnect();
        return true;
    }
...
void WifiConfigurationAp::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        ESP_LOGI(TAG, "STA connected event received, setting WIFI_CONNECTED_BIT");
        xEventGroupSetBits(self->event_group_, WIFI_CONNECTED_BIT);
    } else if (event_id == WIFI_EVENT_STA_DISCONNECTED) {
        wifi_event_sta_disconnected_t* event = (wifi_event_sta_disconnected_t*) event_data;
        ESP_LOGW(TAG, "STA disconnected, reason=%d, setting WIFI_FAIL_BIT", event->reason);
        xEventGroupSetBits(self->event_group_, WIFI_FAIL_BIT);
    } else if (event_id == WIFI_EVENT_STA_START) {
        ESP_LOGI(TAG, "STA started event received");
...
void WifiConfigurationAp::IpEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == IP_EVENT_STA_GOT_IP) {
        ...
        ESP_LOGI(TAG, "Got IP:" IPSTR ", setting WIFI_CONNECTED_BIT", IP2STR(&event->ip_info.ip));
        xEventGroupSetBits(self->event_group_, WIFI_CONNECTED_BIT);
