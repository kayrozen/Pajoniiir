...
typedef struct
{
    ...
    TickType_t      last_frame_time;  /* Last frame timestamp */
    uint32_t        frame_interval_ms;/* Frame interval in milliseconds */
...
static lv_obj_t * create_lvgl_objects(lv_obj_t * screen)
{
    ...
    lv_obj_set_size(cont_col, player_ctx.screen_width,
                    player_ctx.screen_height - (player_ctx.hide_controls ? 0 : 80));
...
static void show_video_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "Target FPS: %d, Frame interval: %d ms", player_ctx.target_fps, player_ctx.frame_interval_ms);
    ...
    player_ctx.last_frame_time = xTaskGetTickCount();
    ...
    while (player_ctx.state != PLAYER_STATE_STOPPED) {
        ...
        if (jpeg_image_size < 0) {
            ...
            if (player_ctx.loop) {
                ...
                file_seek_offset = 0;
                all_size = 0;
                player_ctx.last_frame_time = xTaskGetTickCount();  // Reset timing for loop
                continue;
            } else {
                esp_lvgl_simple_player_stop();
                continue;
        ...
        TickType_t elapsed_time = current_time - player_ctx.last_frame_time;
        
        // Calculate remaining time to wait to achieve target FPS
        if (elapsed_time < pdMS_TO_TICKS(player_ctx.frame_interval_ms)) {
            TickType_t wait_time = pdMS_TO_TICKS(player_ctx.frame_interval_ms) - elapsed_time;
...
lv_obj_t * esp_lvgl_simple_player_create(esp_lvgl_simple_player_cfg_t * params)
{
    ...
    player_ctx.target_fps = 15;
    player_ctx.frame_interval_ms = 1000 / player_ctx.target_fps;  //  
    player_ctx.last_frame_time = 0;
...
void esp_lvgl_simple_player_change_file(const char *video_file, const char *bgm_file)
{
    if (!player_ctx.is_init) {
        ESP_LOGW(TAG, "Not init");
        return;
    ...
    player_ctx.bgm_path = bgm_file;

    ESP_LOGI(TAG, "Video file changed to %s", video_file);
    if (bgm_file) {
        ESP_LOGI(TAG, "BGM file changed to %s", bgm_file);
    } else {
        ESP_LOGI(TAG, "No BGM file set");
    }
}
...
void esp_lvgl_simple_player_seek(uint32_t position)
{
    if (!player_ctx.is_init) {
        ESP_LOGW(TAG, "Not init");
        return;
    ...
    if (position > 1000) {
        ESP_LOGW(TAG, "Position out of range (0-1000)");
        return;
    }

    // Calculate the target position in file
    uint64_t target_pos = (uint64_t)((float)position / 1000.0f * player_ctx.filesize);
    
    ESP_LOGI(TAG, "Seeking to position: %u (%.1f%%), file offset: %llu", 
             position, (float)position / 10.0f, target_pos);
    
    // Seek to the target position
    media_src_storage_seek(&player_ctx.file, target_pos);
    
    // Update slider value
    bsp_display_lock(0);
    lv_slider_set_value(player_ctx.slider, position, LV_ANIM_OFF);
    bsp_display_unlock();
}
