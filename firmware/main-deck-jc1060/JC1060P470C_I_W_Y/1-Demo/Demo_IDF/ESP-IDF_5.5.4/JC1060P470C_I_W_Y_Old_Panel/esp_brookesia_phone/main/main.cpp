...
const char *SOFTWARE_VERSION = "v2.7.0";
...
// #endif
    ...
    NewApp *new_app = new NewApp(1024, 600);
    assert(new_app != nullptr && "Failed to create new_app");
    assert((phone->installApp(new_app) >= 0) && "Failed to begin new_app");

    TouchDraw *touch_draw = new TouchDraw(1024, 600);
    assert(touch_draw != nullptr && "Failed to create touch_draw");
    assert((phone->installApp(touch_draw) >= 0) && "Failed to begin touch_draw");
